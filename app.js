...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Guardas: si algo crítico no existe, avisamos y no reventamos
  if (!els.tbody || !els.workout) {
    console.error("Faltan elementos del DOM: revisa que existan #tbody y #workout en index.html");
    return;
  }
  ...
  // CSV simple (como lo traías). Si tu CSV llega a tener comas dentro de campos,
  // ahí sí conviene parser más robusto.
  ...
  function renderTop3ByBox(data, view) {
    if (!els.boxes) return; // si no existe la sección, no rompemos
    ...
    els.tbody.innerHTML = filtered.map((a) => {
      ...
      return `
        <tr>
          ...
          <td><small>#${escapeHTML(a.w1_rank || "-")} · ${escapeHTML(a.w1_score || "-")}</small></td>
          <td><small>#${escapeHTML(a.w2_rank || "-")} · ${escapeHTML(a.w2_score || "-")}</small></td>
          <td><small>#${escapeHTML(a.w3_rank || "-")} · ${escapeHTML(a.w3_score || "-")}</small></td>
        </tr>
      `;
    }).join("");
  }
  ...
  async function load() {
    try {
      ...
      console.error(e);
      if (els.lastUpdated) els.lastUpdated.textContent = "Error cargando datos";
    }
  }
